...
static ERL_NIF_TERM cv_monochrome_gpu_nif(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    if(__builtin_expect(argc != 2, false)) {
        return enif_make_badarg(env);
    }
    ...
    cv_monochrome_gpu(vec_size, in, out);
...
static ErlNifFunc nif_funcs[] =
{
	{"cv_monochrome_nif", 2, cv_monochrome_nif},
  	{"cv_monochrome_gpu_nif", 2, cv_monochrome_gpu_nif},
};
